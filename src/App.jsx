...
function calcPlan(p) {
  ...
  const bmi =
    Math.round((p.weightKg / Math.pow(p.heightCm / 100, 2)) * 10) / 10;
  const protein = Math.round((recommended * 0.3) / 4);
  const carbs = Math.round((recommended * 0.4) / 4);
  const fat = Math.round((recommended * 0.3) / 9);
...
const TIMELINES = [
  {
    label: '1 month',
    sub: 'Aggressive — strong deficit',
    months: 1,
    icon: '⚡',
  },
  {
    label: '3 months',
    sub: 'Recommended — healthy pace',
    months: 3,
    icon: '🎯',
  },
  {
    label: '6 months',
    sub: 'Gradual — easy to maintain',
    months: 6,
    icon: '🌱',
  },
];
...
function Step1({ data, set, onNext }) {
  ...
  return (
    <StepShell
      ...
      sub="We'll personalise your plan around your profile"
      onNext={go}
    >
      <Input
        label="First name"
...
function Step2({ data, set, onNext, onBack }) {
  ...
  const go = () => {
    if (!data.dob || !age || age < 10 || age > 110) {
  ...
  return (
    <StepShell
      ...
      sub="Age changes your metabolic rate calculation"
      onNext={go}
      onBack={onBack}
    >
      <Input
...
function Step3({ data, set, onNext, onBack }) {
  ...
  const go = () => {
    if (!data.heightCm || data.heightCm < 100 || data.heightCm > 250) {
      setErr('Please enter a valid height');
      return;
    }
    setErr('');
    onNext();
  ...
  const display = data.heightCm
    ? `${Math.floor(data.heightCm / 30.48)}ft ${Math.round(
  ...
  return (
    <StepShell
      ...
      sub="Used to calculate your BMI and daily energy needs"
      onNext={go}
      onBack={onBack}
    >
      <div style={{ marginBottom: 16 }}>
...
function Step4({ data, set, onNext, onBack }) {
  ...
  const go = () => {
    ...
    if (data.weightKg && data.targetKg && data.weightKg <= data.targetKg)
      e.tw = 'Target must be lower than current for weight loss';
  ...
  const diff =
    data.weightKg && data.targetKg && data.weightKg > data.targetKg
      ? (data.weightKg - data.targetKg).toFixed(1)
      : null;
  ...
  return (
    <StepShell
      ...
      total={5}
      title="Weight & goal"
      sub="We'll compute your exact daily calorie target"
      onNext={go}
      onBack={onBack}
    >
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div>
          <Input
            ...
            value={data.targetKg || ''}
            onChange={(e) => {
              set('targetKg', parseFloat(e.target.value) || '');
              setErrs((p) => ({ ...p, tw: '' }));
            }}
          />
          {errs.tw && (
      ...
      {diff && (
        ...
        >
          Goal: lose <strong style={{ color: '#34d399' }}>{diff} kg</strong>{' '}
          total
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {TIMELINES.map((t) => (
          <Chip
            key={t.months}
            active={data.months === t.months}
            onClick={() => set('months', t.months)}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div
                style={{
                  ...
                  borderRadius: '50%',
                  background:
                    data.months === t.months ? '#f97316' : 'transparent',
                  border:
...
function Step5({ data, set, onNext, onBack }) {
  return (
    <StepShell
      ...
      sub="Determines your total daily energy expenditure (TDEE)"
      onNext={onNext}
      onBack={onBack}
      nextLabel="Calculate my plan ✨"
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {ACTIVITIES.map((a) => (
          <Chip
            key={a.mult}
            active={data.actMult === a.mult}
            onClick={() => {
              set('actMult', a.mult);
              set('actLabel', a.label);
            }}
          >
            <div
              style={{
...
function Results({ profile, plan, onStart }) {
  ...
  const milestones = [
    { month: 0, label: 'Start', weight: profile.weightKg, icon: '🚀' },
  ];
  ...
  for (let m = 1; m <= profile.months; m++) {
    ...
    milestones.push({
      ...
      weight: (profile.weightKg - perMonth * m).toFixed(1),
      icon: m === profile.months ? '🏆' : '📍',
    });
  }
      ...
      <div style={{ maxWidth: 460, margin: '0 auto', padding: '0 20px 60px' }}>
        ...
        <div style={{ textAlign: 'center', padding: '36px 0 24px' }}>
          ...
          <div style={{ fontSize: 14, color: 'rgba(255,255,255,0.4)' }}>
            Lose {lossKg} kg in {profile.months} month
            {profile.months > 1 ? 's' : ''}
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                textAlign: 'center',
                background:
                  chosen === key ? '#f97316' : 'rgba(255,255,255,0.06)',
                outline:
                  chosen === key ? 'none' : '1px solid rgba(255,255,255,0.08)',
                transition: 'all 0.2s',
              }}
            >
              <div
          ...
          {[
            {
              ...
              val: plan.bmr,
              unit: 'kcal',
              note: 'Calories at rest',
            },
            {
              label: 'TDEE',
              val: plan.tdee,
              unit: 'kcal',
              note: 'Total daily burn',
            },
            {
              label: 'Daily deficit',
              val: plan.dailyDeficit,
              unit: 'kcal',
              note: 'Below maintenance',
            },
            {
              label: 'BMI',
              val: plan.bmi,
            ...
            >
              ...
              <div
                style={{
                  fontSize: 11,
                  color: s.noteColor || 'rgba(255,255,255,0.3)',
                  marginTop: 4,
                }}
              >
                {s.note}
        ...
        >
          ...
          {[
            ['Protein', plan.protein, 30, '#60a5fa'],
            ['Carbs', plan.carbs, 40, '#fbbf24'],
            ['Fat', plan.fat, 30, '#f472b6'],
          ].map(([name, g, pct, color]) => (
            <div
              key={name}
              style={{
                display: 'flex',
        ...
        <PrimaryBtn
          onClick={handleStart}
          style={{ padding: '17px 20px', fontSize: 16 }}
        >
          Start tracking →
        </PrimaryBtn>
        <div
        ...
        >
          Based on Mifflin-St Jeor · Adjust anytime in Profile
        </div>
      </div>
    </div>
  );
...
function MainApp({ profile, log, setLog, onReset }) {
      ...
      if (newDay !== currentDay) {
        ...
        try {
          const all = JSON.parse(
            localStorage.getItem('nutriai_activity') || '{}'
          );
          setActivity(all[newDay] || { steps: 0, waterMl: 0 });
        } catch {
          setActivity({ steps: 0, waterMl: 0 });
        }
      }
    };
    // Check every minute
  ...
  const addGlass = () =>
    saveActivity({
      ...activity,
      waterMl: Math.min(WATER_GOAL_ML * 2, activity.waterMl + GLASS_ML),
    });
  const removeGlass = () =>
    saveActivity({
      ...activity,
      waterMl: Math.max(0, activity.waterMl - GLASS_ML),
    });
  ...
  const tmp = new Date();
  for (let i = 0; i < 365; i++) {
    const k = `${tmp.getFullYear()}-${String(tmp.getMonth() + 1).padStart(
      2,
      '0'
  ...
  const onGoalDays = Object.keys(log).filter((k) => {
    const dc = (log[k] || []).reduce((s, e) => s + e.calories, 0);
    return dc > 0 && dc <= profile.goal;
  }).length;
  ...
  const progressPct =
    totalLoss > 0
      ? Math.min(100, ((startW - profile.weightKg) / totalLoss) * 100)
      : 0;
  ...
  const weekBars = Array.from({ length: 7 }, (_, i) => {
    ...
    return {
      ...
      cal: (log[k] || []).reduce((s, e) => s + e.calories, 0),
      isToday: k === today,
    };
  });
  ...
  const analyse = async () => {
    ...
    try {
      ...
      setError('Could not analyse. ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const removeEntry = (id) =>
    setLog({ ...log, [today]: (log[today] || []).filter((e) => e.id !== id) });

  const updateEntry = (id, changes) => {
    const updated = {
      ...log,
      [today]: (log[today] || []).map((e) =>
        e.id === id ? { ...e, ...changes } : e
      ),
    };
    setLog(updated);
        ...
        <div style={S.inner}>
          ...
          <div style={S.darkCard}>
            ...
            >
              <Ring
                ...
                strokeW={9}
                pct={ringPct}
                color={ringPct > 100 ? '#f87171' : '#f97316'}
                bg="rgba(255,255,255,0.06)"
              >
                <div style={{ fontSize: 18, fontWeight: 800, color: '#fff' }}>
                  {Math.min(ringPct, 999)}
              ...
              <div>
                ...
                >
                  Goal: <span style={{ color: '#f97316' }}>{adjustedGoal}</span>
                  {stepsBurned > 0 && (
            ...
            >
              {[
                ['Protein', totalP, '#60a5fa', profile.protein],
                ['Carbs', totalC, '#fbbf24', profile.carbs],
                ['Fat', totalF, '#f472b6', profile.fat],
              ].map(([name, val, color, target]) => (
                <div
                  key={name}
                  style={{
                    background: 'rgba(255,255,255,0.06)',
          ...
          <div style={S.card}>
            ...
            >
              ...
              <div style={{ textAlign: 'center' }}>
                <div
                  style={{ fontSize: 20, fontWeight: 800, color: '#34d399' }}
                >
                  {profile.targetKg}
                </div>
                <div
            ...
            >
              ...
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)' }}>
                ~
                {(
                  (profile.weightKg - profile.targetKg) /
                  Math.max(1, daysLeft / 7)
                ).toFixed(1)}{' '}
                kg/week
              </div>
            </div>
          ...
          <div style={S.card}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <button
                    onClick={() => {
                      setStepsInput(String(activity.steps));
              ...
              <div style={{ marginBottom: 4 }}>
                <ProgressBar
                  pct={Math.round((activity.steps / 10000) * 100)}
                  color="#34d399"
                  h={6}
                />
              </div>
            ...
            {showStepsModal && (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    onClick={() => {
                      saveActivity({
                        ...activity,
                        steps: Math.max(0, parseInt(stepsInput) || 0),
                      });
                ...
                >
                  {Math.max(0, parseInt(stepsInput) || 0).toLocaleString()}{' '}
                  steps = +
                  {stepCalsBurned(Math.max(0, parseInt(stepsInput) || 0))} kcal
                  · New goal:{' '}
                  {profile.goal +
                    stepCalsBurned(Math.max(0, parseInt(stepsInput) || 0))}{' '}
                  kcal
                </div>
              </div>
            )}
            ...
            <div>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <div>
                    ...
                    <div
                      style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)' }}
                    >
                      {activity.waterMl} ml of {WATER_GOAL_ML} ml goal
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  >
                    {Math.round(activity.waterMl / GLASS_ML)}
                  </div>
                  <button
                    onClick={addGlass}
                    style={{
              ...
              >
                {Array.from({
                  length: Math.round(WATER_GOAL_ML / GLASS_ML),
                }).map((_, i) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        border:
                          '1.5px solid ' +
                          (filled ? '#60a5fa' : 'rgba(255,255,255,0.1)'),
                        background: filled
                          ? 'rgba(96,165,250,0.2)'
              ...
              <ProgressBar
                pct={Math.round((activity.waterMl / WATER_GOAL_ML) * 100)}
                color="#60a5fa"
                h={5}
              />
            </div>
          ...
          >
            {!preview ? (
              ...
              >
                ...
                >
                  AI identifies calories & macros instantly
                </div>
                <button
                  style={{
                    background: 'linear-gradient(135deg,#f97316,#ea580c)',
          ...
          {scanResult && (
            <div style={S.card}>
              ...
              >
                {[
                  ['Protein', scanResult.protein_g, '#60a5fa'],
                  ['Carbs', scanResult.carbs_g, '#fbbf24'],
                  ['Fat', scanResult.fat_g, '#f472b6'],
                ].map(([n, v, c]) => (
                  <div
                    key={n}
                    style={{
                ...
                >
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: '50%',
                      background: MEAL_COLORS[e.meal] || '#888',
                      flexShrink: 0,
                    }}
                  />
                  <div style={{ flex: 1 }}>
                  ...
                  <div
                    style={{ fontWeight: 800, fontSize: 14, color: '#f97316' }}
                  >
                    {e.calories}
                  </div>
                  {/* Edit button */}
                  <button
                    onClick={() =>
                      setEditEntry(editEntry?.id === e.id ? null : { ...e })
                    }
                    style={{
                      background:
                        editEntry?.id === e.id
                  ...
                  >
                    ...
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <input
                        ...
                        onChange={(ev) =>
                          setEditEntry((p) => ({ ...p, name: ev.target.value }))
                        }
                        style={{
                          width: '100%',
                          padding: '9px 12px',
                    ...
                    <div style={{ marginBottom: 12 }}>
                      ...
                      <input
                        ...
                        onChange={(ev) =>
                          setEditEntry((p) => ({
                            ...p,
                            calories: parseInt(ev.target.value) || 0,
                          }))
                        }
                        style={{
                          width: '100%',
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button
                        onClick={() => updateEntry(e.id, editEntry)}
                        style={{
                          flex: 1,
                          background: 'linear-gradient(135deg,#f97316,#ea580c)',
                          color: '#fff',
        ...
        <div style={S.inner}>
              ...
              <div style={{ flex: 1 }}>
                <div
                  style={{
                    ...
                    borderRadius: 99,
                    background:
                      'linear-gradient(90deg,#60a5fa 0%,#34d399 25%,#fbbf24 60%,#f87171 100%)',
                    marginBottom: 8,
                    position: 'relative',
                  }}
                >
          ...
          >
            {[
              ...
              {
                icon: '⚖️',
                val: `${(profile.weightKg - profile.targetKg).toFixed(1)} kg`,
                label: 'To goal',
              },
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      style={{
                        fontSize: 10,
                        color: b.isToday ? '#f97316' : 'rgba(255,255,255,0.3)',
      ...
      {tab === 'profile' && (
        <div style={S.inner}>
          ...
          >
            ...
            >
              {profile.weightKg} kg → {profile.targetKg} kg in {profile.months}{' '}
              month{profile.months > 1 ? 's' : ''}
            </div>
            <div
              style={{
                display: 'grid',
            ...
            >
              {[
                ...
                [
                  'Height',
                  Math.floor(profile.heightCm / 30.48) +
                    'ft ' +
            ...
            {[
              {
                ...
                val: profile.goal + ' kcal',
                bg: 'rgba(249,115,22,0.15)',
              },
              {
                icon: '⚖️',
                label: 'Current weight',
                val: profile.weightKg + ' kg',
                bg: 'rgba(52,211,153,0.12)',
              },
              {
                icon: '🏁',
                label: 'Target weight',
                val: profile.targetKg + ' kg',
                bg: 'rgba(96,165,250,0.12)',
              },
              {
                icon: '🏃',
                label: 'Activity level',
                val: profile.actLabel,
                bg: 'rgba(251,191,36,0.12)',
              },
              {
                icon: '📅',
                label: 'Goal deadline',
                val: deadline.toLocaleDateString('en-GB', {
                  day: 'numeric',
                  month: 'short',
                  year: 'numeric',
                }),
...
export default function App() {
  ...
  if (phase === 'onboard')
    return (
      <>
        {step === 1 && (
          <Step1 data={form} set={setField} onNext={() => setStep(2)} />
        )}
        {step === 2 && (
          <Step2
            data={form}
  ...
  if (phase === 'results')
    return (
      <Results
        profile={{ ...form, age: calcAge(form.dob) }}
        plan={plan}
        onStart={launchApp}
      />
    );
  if (phase === 'app')
    return (
      <MainApp profile={profile} log={log} setLog={setLog} onReset={reset} />
    );
  return null;
}
